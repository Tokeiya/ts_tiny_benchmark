...
export function ddtostring(
	...
	if (isNaN(x1) || isNaN(x2)) return 'nan';
	...
	let buf: number[] = new Array(1023 - -1074 + 2).fill(0);
	const offset: number = 1074;
	let emax: number = 0,
		emin: number = 0;
	...
	let buf2: number[] = new Array(1023 - -1074 + 1).fill(0);
	//int emax2, emin2, s;
	let emax2: number = 0,
		emin2: number = 0,
		s: number;
	...
	let result1: Dequeue<number> = new Dequeue<number>();
	let result2: Dequeue<number> = new Dequeue<number>();
	...
	let result = new Array(result_max - result_min + 1 + 2).fill(0);
